...
sep_old <- 
  ...
  # We want to group the classes and then find the sum
...
sep_new <-
  ...
  rename(id = course_id,
         ...
         u_grad2 = u_grad) %>% 
  group_by(id, title, name, department) %>%
  summarize(u_grad2 = sum(u_grad2), .groups = "drop") %>%
  filter(u_grad2 > 10) %>%
  arrange(department)
...
```{r}
# Decide what data is relevant to the problem itself.

# Throw out any duplicate rows or rows that are outside the scope 
